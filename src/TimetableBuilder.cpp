...
std::optional<Timetable> TimetableBuilder::buildTimetable(std::vector<Course*>& courses, uint32_t* checked) {
  ...
  while (!currentTimetables.empty() && currentTimetables[0].coursesPlaced < courseCount) {
    ...
    for (Timetable& timetable : currentTimetables) {
      ...
      for (size_t i = 0; i < courseToPlace->sections.size(); i++) {
        ...
        if (timetable.isConflicting(*section))
          continue;
        ...
        if (newTimetable.fitnessScore > FITNESS_PRUNE_THRESHOLD)
          newCandidates.push_back(newTimetable);
      }
    }

    if (newCandidates.empty() && currentTimetables.empty())
      return std::nullopt;
    ...
    std::partial_sort(newCandidates.begin(), newCandidates.begin() + end, newCandidates.end(),
  ...
  if (checked)
    *checked = timetablesChecked;
  return currentTimetables[0];
}

uint32_t TimetableBuilder::getPlacementScore(const Timetable& timetable, const std::vector<Course*>& courses,
                                             int index) {
...
int TimetableBuilder::getFitnessScoreForDay(const Timetable& timetable, const std::vector<Course*>& courses, int day,
                                            int semester) {
  int fitness = 0;

  auto meetingTimes = timetable.inPersonMeetingTimes[semester][day] | timetable.onlineMeetingTimes[semester][day];
  uint32_t startTimeslot =
      std::min(timetable.inPersonFirstMeetingTimes[semester][day], timetable.onlineFirstMeetingTimes[semester][day]);
  uint32_t endTimeslot =
      std::max(timetable.inPersonLastMeetingTimes[semester][day], timetable.onlineLastMeetingTimes[semester][day]);
  ...
  if (endTimeslot >= startTimeslot) {
    ...
    if (daySpan > MAX_DAY_LENGTH * TIMESLOTS_PER_HOUR) {
      fitness -= MAX_DAY_LENGTH_PENALTY *
                 pow(timeslotsToHours(daySpan - MAX_DAY_LENGTH * TIMESLOTS_PER_HOUR), MAX_DAY_LENGTH_PENALTY_EXPONENT);
    }
    if (daySpan < MIN_DAY_LENGTH * TIMESLOTS_PER_HOUR) {
      fitness -= MIN_DAY_LENGTH_PENALTY *
                 pow(timeslotsToHours(MIN_DAY_LENGTH * TIMESLOTS_PER_HOUR - daySpan), MIN_DAY_LENGTH_PENALTY_EXPONENT);
    }
  }
  ...
  const int startScore = (int)std::lround(std::max(0.0, (double)PREFERRED_MIN_START_REWARD - startDiffHours));

  const double lateEndHours = std::max(0.0, endSlot - prefEndSlot) / TIMESLOTS_PER_HOUR;
  const int endScore = (int)std::lround(std::max(0.0, (double)PREFERRED_MAX_END_REWARD - lateEndHours));
  ...
  for (uint32_t timeslot = startTimeslot; timeslot <= endTimeslot; timeslot++) {
    ...
    if (lastTimeslot != -1) {
      ...
      if (gap > MAX_GAP * TIMESLOTS_PER_HOUR) {
        fitness -=
            MAX_GAP_PENALTY * pow(timeslotsToHours(gap - MAX_GAP * TIMESLOTS_PER_HOUR), MAX_GAP_PENALTY_EXPONENT);
      }

      if (!lastSection || !lastCourse)
        continue;
      ...
      if (GUARANTEE_CROSS_CAMPUS_GAP && !lastOnline && !currentOnline && lastCourse->campus != course->campus &&
  ...
  if (maxBlock > MAX_CONTINUOUS_CLASSES * TIMESLOTS_PER_HOUR) {
    fitness -=
        MAX_CONTINUOUS_CLASSES_PENALTY * pow(timeslotsToHours(maxBlock - MAX_CONTINUOUS_CLASSES * TIMESLOTS_PER_HOUR),
                                             MAX_CONTINUOUS_CLASSES_PENALTY_EXPONENT);
  }
  ...
  if (AVOID_RUSH_HOURS) {
    ...
    for (auto it = std::begin(RUSH_HOURS); it != std::end(RUSH_HOURS); it++) {
      if (*it == startHour || *it == endHour)
        fitness -= AVOID_RUSH_HOUR_PENALTY;
    }
  }
  ...
  if (ONLINE_PREFERENCE != 2) {
    ...
    if (totalCountDay > 0) {
      ...
      if (ONLINE_PREFERENCE == 0) {
        score = (uint32_t)std::lround((double)(ONLINE_PREFERENCE_REWARD * (totalCountDay - onlineCountDay)) /
                                      totalCountDay);
      } else {
...
int TimetableBuilder::getFitnessScoreForWeek(const Timetable& timetable, int semester) {
  ...
  for (uint32_t day = 0; day < DAYS_PER_WEEK; day++) {
    if ((timetable.inPersonMeetingTimes[semester][day] | timetable.onlineMeetingTimes[semester][day]).any()) {
      int start = (int)std::min(timetable.inPersonFirstMeetingTimes[semester][day],
                                timetable.onlineFirstMeetingTimes[semester][day]);
      int end = (int)std::max(timetable.inPersonLastMeetingTimes[semester][day],
                              timetable.onlineLastMeetingTimes[semester][day]);
  ...
  if (minDaySpan == INT32_MAX)
    minDaySpan = 0;
  if (maxDaySpan == INT32_MIN)
    maxDaySpan = 0;

  if ((uint32_t)maxDaySpan - (uint32_t)minDaySpan <= BALANCED_WEEK_DELTA * TIMESLOTS_PER_HOUR)
    return BALANCED_WEEK_DELTA_REWARD;
  return 0;
}

void TimetableBuilder::addSectionAndUpdateFitness(Timetable& timetable, const Section* section,
  ...
  for (uint32_t semester = 0; semester < MAX_SEMESTERS; semester++) {
    for (uint32_t day = 0; day < DAYS_PER_WEEK; day++) {
      if (section->inPersonMeetingTimes[semester][day].any())
        timetable.hasInPersonClass[semester][day] = true;
      ...
      if (inPersonStart != -1)
        timetable.inPersonFirstMeetingTimes[semester][day] =
            std::min(timetable.inPersonFirstMeetingTimes[semester][day], (uint32_t)inPersonStart);
      if (inPersonEnd != -1)
        timetable.inPersonLastMeetingTimes[semester][day] =
            std::max(timetable.inPersonLastMeetingTimes[semester][day], (uint32_t)inPersonEnd);
      if (onlineStart != -1)
        timetable.onlineFirstMeetingTimes[semester][day] =
            std::min(timetable.onlineFirstMeetingTimes[semester][day], (uint32_t)onlineStart);
      if (onlineEnd != -1)
        timetable.onlineLastMeetingTimes[semester][day] =
            std::max(timetable.onlineLastMeetingTimes[semester][day], (uint32_t)onlineEnd);
    }
  }
...
int TimetableBuilder::getCourseIndexAtSlot(const Timetable& timetable, const std::vector<Course*>& courses, int day,
                                           int semester, int timeslot) {
  for (uint32_t courseIndex = 0; courseIndex < (uint32_t)courses.size(); courseIndex++) {
    ...
    if (section->inPersonMeetingTimes[semester][day].test(timeslot) ||
...
double TimetableBuilder::timeslotsToHours(uint32_t timeslots) { return (double)timeslots / TIMESLOTS_PER_HOUR; }