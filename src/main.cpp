...
int main() {
  ...
  const std::unordered_map<std::string, int> semesterBySession = {{"20259", 0}, {"20261", 1}};
  ...
  const std::vector<std::pair<std::string, std::string>> selectedCourses = {
      {"CSC263H5", "S"}, {"CSC209H5", "S"}, {"MAT224H5", "S"}, {"PHL245H5", "S"}, {"MAT232H5", "S"}};
  //=====
  ...
  for (const json* courseData : selectedCourseData) {
    ...
    for (const auto& sectionData : (*courseData)["sections"]) {
      ...
      const std::string typeCode = sectionType == "Lecture"     ? "LEC"
      ...
      for (const auto& meetingTimeData : sectionData["meetingTimes"]) {
        ...
        if (semIt == semesterBySession.end())
          continue;
      ...
      sectionMeetingTimesByChoice[std::string((*courseData)["code"]) + "|" + typeCode + "|" +
                                  std::string(sectionData["name"])] = normalizedMeetingTimes;

      if (typeCode == "LEC")
        lecturesJSON["sections"].push_back(sectionJSON);
      else if (typeCode == "TUT")
        tutorialsJSON["sections"].push_back(sectionJSON);
      else if (typeCode == "PRA")
        practicalsJSON["sections"].push_back(sectionJSON);
    }
  ...
  for (const auto& entry : result) {
    ...
    for (const auto& meetingTimeData : lookupIt->second) {
      std::cout << "  Semester: " << (int)meetingTimeData["semester"] << "  Day: " << meetingTimeData["day"]
                << ", Start: " << (int)meetingTimeData["start"] / 3600 << ":" << std::setw(2) << std::setfill('0')
                << ((int)meetingTimeData["start"] % 3600) / 60 << ", End: " << (int)meetingTimeData["end"] / 3600 << ":"
