...
private:
  /**
   * @brief Calculates the priority of a specific course being placed based on how many possible sections it has, and
   * how many of those cause conflicts. Smaller priorities (more possible options) are placed later on
   ...
   */
  static uint32_t getPlacementScore(const Timetable& timetable, const std::vector<Course*>& courses, int index);
   ...
   */
  static int getFitnessScoreForDay(const Timetable& timetable, const std::vector<Course*>& courses, int day,
                                   int semester);
   ...
   * @param courses The courses being placed
   */
  static void addSectionAndUpdateFitness(Timetable& timetable, const Section* section,
                                         const std::vector<Course*>& courses);

  /**
   * @brief Returns the index into the courses vector based on what course is blocking a specific timeslot in a
   * timetable
   ...
   */
  static int getCourseIndexAtSlot(const Timetable& timetable, const std::vector<Course*>& courses, int day,
