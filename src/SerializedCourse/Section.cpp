...
void Section::addMeetingTime(size_t startSeconds, size_t endSeconds, int day, bool online, bool zz, int semester) {
  ...
  for (size_t i = startPos; i < endPos; i++) {
    if (online)
      this->onlineMeetingTimes[semester][day].set(i);
    else if (zz)
      this->zzMeetingTimes[semester][day].set(i);
    else
      this->inPersonMeetingTimes[semester][day].set(i);
  }
}
  ...
  for (uint32_t day = 0; day < DAYS_PER_WEEK; day++) {
    for (uint32_t semester = 0; semester < MAX_SEMESTERS; semester++) {
      const auto thisRegularMeetings =
          this->inPersonMeetingTimes[semester][day] | this->onlineMeetingTimes[semester][day];
      const auto otherRegularMeetings =
          other.inPersonMeetingTimes[semester][day] | other.onlineMeetingTimes[semester][day];
      ...
      if ((thisRegularMeetings & otherRegularMeetings).any())
        return true;

      // Regular/zz conflicts
      if ((thisRegularMeetings & otherZzMeetings).any())
        return true;
      if ((otherRegularMeetings & thisZzMeetings).any())
        return true;
    }
  }
...
int Section::getInPersonStart(int day, int semester) const {
  for (uint32_t i = 0; i < TIMESLOTS_PER_DAY; i++) {
    if (this->inPersonMeetingTimes[semester][day].test(i))
      return i;
  }
...
int Section::getOnlineStart(int day, int semester) const {
  for (uint32_t i = 0; i < TIMESLOTS_PER_DAY; i++) {
    if (this->onlineMeetingTimes[semester][day].test(i))
      return i;
  }
